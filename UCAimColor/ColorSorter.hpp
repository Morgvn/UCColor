...
#include <mutex>  // Para prote��o de threads
#
...
void ProcessImage(BYTE* screenData, int w, int h) {
    ...
    // Variables para encontrar el pixel más cercano al centro
    int closestX = 0;
    int closestY = 0;
    const int MAX_INT_VALUE = 2147483647; // Valor m�ximo para um int de 32 bits
    ...
    for (int j = 0; j < h; ++j) {
        for (int i = 0; i < w * 4; i += 4) {
            ...
            if (PS.IsPurpleColor(red, green, blue)) {
                // Calcular distancia desde el centro con offsets aplicados
                int dx = (i / 4) - (w / 2) + cfg::head_offset_x;
                int dy = j - (h / 2) + cfg::head_offset_y;
                int distanceSquared = dx * dx + dy * dy;
                
                // CORRECCIÓN: Encontrar el pixel MÁS CERCANO al centro (no el más alto)
                if (distanceSquared < closestDistanceSquared) {
                    closestDistanceSquared = distanceSquared;
                    closestX = dx;
                    closestY = dy;
                    
                    // También guardar coordenadas globales y para silent aim
        ...
        // Encontre o pixel mais pr�ximo em termos de coordenada X
        // Encontre o pixel roxo mais alto (comece do topo e v� at� o final horizontalmente)
 // Inicialize com o valor m�ximo (altura da tela)
...
void CaptureScreen() {
    ...
    while (true) {
        int w, h;
        {
            ...
            // Inicializar FOV si está en 0
            if (currentFOV == 0) {
                currentFOV = cfg::aimbot_fov > 0 ? cfg::aimbot_fov : 35; // Valor por defecto
            }
            w = currentFOV > 0 ? currentFOV : 35;
            h = currentFOV > 0 ? currentFOV : 35;
        }
        
        // Asegurar que w y h sean válidos
        if (w <= 0 || h <= 0) {
            w = 35;
            h = 35;
        }
        ...
        if (!hScreen) {
            ...
            continue; // Saltar esta iteración si falla GetDC
        }
        ...
        // CORRECCIÓN CRÍTICA: Calcular tamaño correcto del buffer (w * h * 4 bytes para 32 bits)
        ...
        if (GetDIBits(hDC, hBitmap, 0, h, screenData, &bmi, DIB_RGB_COLORS) == 0) {
            // Error al obtener bits, limpiar y continuar
            free(screenData);
            DeleteObject(hBitmap);
            DeleteDC(hDC);
            ReleaseDC(NULL, hScreen);
        ...
        // Ejecutar aimbot después de procesar la imagen (cuando ya tenemos aim_x y aim_y)
        // Siempre se ejecuta, sin condiciones que lo desactiven
        ...
        // Solo aplicar sleep si está configurado (por defecto es 0, así que no hay delay)
        if (cfg::sleep > 0) {
