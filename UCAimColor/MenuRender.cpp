...
void render_menu::create_imgui()
{
	...
	// Cargar logo usando stb_image y DirectX 11
	logo = nullptr; // Asegurar que está inicializado
	// Intentar cargar el logo solo si el dispositivo está disponible
	// Nota: Si la carga falla, el logo simplemente no se mostrará (no causará crash)
	if (device != nullptr) {
		try {
			int logo_width = 0, logo_height = 0, logo_channels = 0;
			constexpr int logomain_size = 3521; // Tamaño fijo del array logomain
			unsigned char* logo_data = stbi_load_from_memory(logomain, logomain_size, &logo_width, &logo_height, &logo_channels, 4);
			if (logo_data != nullptr && logo_width > 0 && logo_height > 0 && logo_width < 10000 && logo_height < 10000) {
				...
				subResource.pSysMem = logo_data;
				subResource.SysMemPitch = desc.Width * 4;
				subResource.SysMemSlicePitch = 0;
				if (device->CreateTexture2D(&desc, &subResource, &pTexture) == S_OK && pTexture != nullptr) {
					...
					if (device->CreateShaderResourceView(pTexture, &srvDesc, &logo) != S_OK) {
						logo = nullptr;
					}
					pTexture->Release();
				}
			...
			} else if (logo_data != nullptr) {
				stbi_image_free(logo_data);
			}
		}
		catch (...) {
			// Si hay algún error, simplemente no cargar el logo
			logo = nullptr;
		}
	}
...
char Key[128] = "";
bool DeleteConfigFile(const char* filePath) {
...
void Menu::DrawColor_AndConfig()
{
	...
	ImGui::BeginFeaturesChild("Manage Config", ImVec2(ChildWidht - 25, ChildHeight50 - 37));
	{
		...
		if (ImGui::Button(xorstr_("Excluir Config"), ImVec2(120, 25)) && selectedConfigIndex >= 0 && selectedConfigIndex < configs.size()) {
			...
			if (DeleteConfigFile(filePath.c_str())) {
	...
	ImGui::BeginFeaturesChild("Infos", ImVec2(ChildWidht - 25, ChildHeight50 - 37));
	{
		...
		if (isAdmin) {
			...
			ImGui::Text("  Running as Admin");
			ImGui::PopStyleColor();
		}
		else {
			ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.5f, 0.0f, 1.0f)); // Naranja/Amarillo
			ImGui::Text("  Not Admin (Required!)");
			ImGui::PopStyleColor();
		}
		...
		ImGui::SetCursorPosX(10);
		bool driverConnected = XD.IsConnected();
		if (driverConnected) {
			ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.0f, 1.0f, 0.0f, 1.0f)); // Verde
			...
			ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.0f, 0.0f, 1.0f)); // Rojo
			ImGui::Text("  Not Connected");
			ImGui::PopStyleColor();
			ImGui::SetCursorPosX(10);
			ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 5);
			ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.65f, 0.0f, 1.0f)); // Naranja/Amarillo
			ImGui::Text("  Using Fallback Mode");
			ImGui::PopStyleColor();
			ImGui::SetCursorPosX(10);
			ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 2);
			ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.8f, 0.8f, 0.8f, 1.0f)); // Gris claro
			ImGui::Text("  (More detectable)");
			ImGui::PopStyleColor();
		}
		...
		try {
			if (GetModuleFileNameA(NULL, exePath, MAX_PATH) > 0) {
				...
				std::string mapperPath = exeDir + "Mapper.exe";
				std::string driverPath = exeDir + "Oykyo.sys";
				filesExist = std::filesystem::exists(mapperPath) && std::filesystem::exists(driverPath);
			}
		}
		catch (...) {
			filesExist = false; // Si hay error, asumir que no existen
		}
		
		ImGui::SetCursorPosX(10);
		ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 10);
		ImGui::Text("Files Status:");
		...
		ImGui::SetCursorPosX(10);
		ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 10);
		if (ImGui::Button("Reconnect Driver", ImVec2(150, 25))) {
			// Ejecutar en thread separado para no bloquear el UI
			std::thread([]() {
				XD.TryInitDriver();
			}).detach();
		}
...
void render_menu::render()
{
	...
	ImGui::Begin(xorstr_("hack jheremy free"), NULL,ImGuiWindowFlags_NoMove |ImGuiWindowFlags_NoTitleBar |ImGuiWindowFlags_NoScrollbar |ImGuiWindowFlags_NoCollapse |ImGuiWindowFlags_NoResize);
	{
