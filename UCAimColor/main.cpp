...
std::string get_random_process_name()
{
    ...
    const int length = std::rand() % 21 + 10; // Gera um comprimento aleat�rio entre 10 e 30 caracteres
    ...
    while (true) {
        if (cfg::silent_ativo) {
            if ((GetAsyncKeyState)(cfg::silent_key)) {
                ...
                distance = min(distance, 10.0f);  // Limita o valor m�ximo de distance para 10
                ...
                if (std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - lastFlick).count() > delay) {
                    ...
                    // Ajusta a dist�ncia com base em underflicking e overflicking
                    ...
                    // Multiplica os componentes normalizados pela dist�ncia ajustada
...
int main()
{
    ...
    // Inicializar el driver en un thread separado para que no bloquee el inicio
    std::thread([]() {
        try {
            std::this_thread::sleep_for(std::chrono::milliseconds(500)); // Pequeño delay inicial
            XD.TryInitDriver();
        }
        catch (...) {
            // Si falla la inicialización del driver, continuar de todos modos
        }
    }).detach();
